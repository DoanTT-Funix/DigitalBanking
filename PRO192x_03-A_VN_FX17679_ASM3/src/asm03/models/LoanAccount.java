...
public class LoanAccount extends Account implements Withdraw, ReportService {
    ...
    public boolean isAccepted(double amount) {
        // Không cần ràng buộc hạn mức 100tr, vì
        //Chỉ có khởi tạo 100tr, và không có chức năng nạp tiền
        double soDuSauRut = super.getBalance() - amount - amount * (isPremium() ? LOAN_ACCOUNT_WITHDRAW_PREMIUM_FEE : LOAN_ACCOUNT_WITHDRAW_FEE);
        if (soDuSauRut < 50000) {
            System.out.println("Số tiền rút không hợp lệ ! (Tài khoản sau khi rút PHẢI >= 50_000 VND.)");
            return false;
        }
        return true;
    }

    // In thông tin của 1 account
    @Override
